  ...
  return (
    <main dir="ltr" className="h-screen bg-black text-white flex flex-col justify-between py-6 px-4 relative overflow-hidden" 
        ...
        <div className={`w-full bg-[#0E0E0E] border border-white/5 rounded-[2rem] p-4 md:p-8 shadow-2xl transition-all duration-500 ${file ? 'max-w-[950px]' : 'max-w-[420px]'}`}>
          {!file ? (
            <label className="flex flex-col items-center justify-center w-full h-64 border-2 border-dashed border-white/10 rounded-3xl cursor-pointer hover:border-[#39FF14]/40 transition-all group">
              <Upload className="text-[#39FF14] mb-4 group-hover:scale-110" size={28} />
            ...
            <div className="flex flex-col space-y-4">
              <div className="flex justify-between items-center bg-[#151515] p-2 rounded-xl border border-white/5">
                <button onClick={() => window.location.reload()} className="p-2 text-red-500 hover:bg-red-500/10 rounded-lg"><Trash2 size={16} /></button>
                <div className="flex items-center gap-4 bg-black/40 px-4 py-2 rounded-lg border border-white/5">
                  <button onClick={() => changePage(-1)} disabled={currentPage === 1} className="disabled:opacity-20 hover:text-[#39FF14]"><ChevronLeft size={18} /></button>
                  <span className="text-[9px] font-bold uppercase w-16 text-center">Page {currentPage} / {numPages}</span>
                  <button onClick={() => changePage(1)} disabled={currentPage === numPages} className="disabled:opacity-20 hover:text-[#39FF14]"><ChevronRight size={18} /></button>
                </div>
                <div className="flex gap-2">
                  <button onClick={() => setEditMode('text')} className={`px-4 py-2 rounded-lg transition-all flex items-center gap-2 ${editMode === 'text' ? 'bg-[#39FF14] text-black shadow-[0_0_15px_rgba(57,255,20,0.3)]' : 'hover:bg-white/5 text-white/40'}`}>
                    <Type size={14} /> <span className="text-[10px] font-bold uppercase">Text</span>
                  </button>
                  <button onClick={() => {setEditMode('draw'); setActiveId(null);}} className={`px-4 py-2 rounded-lg transition-all flex items-center gap-2 ${editMode === 'draw' ? 'bg-[#39FF14] text-black shadow-[0_0_15px_rgba(57,255,20,0.3)]' : 'hover:bg-white/5 text-white/40'}`}>
                    <PenTool size={14} /> <span className="text-[10px] font-bold uppercase">Sign</span>
                  </button>
                </div>
              </div>

              <div className="w-full h-[60vh] md:h-[65vh] bg-[#D1D1D1] rounded-xl relative overflow-auto flex justify-center p-4 shadow-inner" onClick={(e) => {
                if (!canvasRef.current || (e.target as HTMLElement).tagName === 'INPUT') return;
                if (editMode === 'text') {
                  ...
                  setActiveId(newId); setEditMode(null);
                ...
                <div className="relative h-max pointer-events-none">
                  ...
                  {placedTexts.filter(t => t.page === currentPage).map(t => (
                    <div key={t.id} onMouseDown={(e) => { isDragging.current = true; setActiveId(t.id); dragOffset.current = { x: e.clientX - t.x, y: e.clientY - t.y }; e.stopPropagation(); }}
                         className={`absolute flex items-center p-3 pointer-events-auto ${activeId === t.id ? 'border-2 border-dashed border-[#39FF14] bg-[#39FF14]/5 z-30' : 'border-2 border-transparent z-20 cursor-pointer'}`}
                         style={{ left: t.x - 12, top: t.y - 12 }}>
                      {activeId === t.id && <div className="absolute -top-6 -left-2 bg-[#39FF14] p-1 rounded text-black shadow-lg cursor-move"><Move size={10} /></div>}
                      <input autoFocus={activeId === t.id} type="text" value={t.text} onChange={(e) => setPlacedTexts(prev => prev.map(pt => pt.id === t.id ? {...pt, text: e.target.value} : pt))} className="bg-transparent border-none outline-none text-black font-bold p-0 m-0 w-full" style={{ fontSize: `${t.size}px`, lineHeight: 1.0, width: `${(t.text.length || 1) + 1}ch` }} />
                    </div>
                  ))}
                  </div>
                </div>
              ...
              <button onClick={finalizeAndDownload} className="w-full bg-[#39FF14] text-black font-black py-4 md:py-5 rounded-2xl flex items-center justify-center gap-3 transition-all uppercase tracking-[0.2em] text-[10px] shadow-[0_10px_30px_rgba(57,255,20,0.2)]">
      ...
      <footer className="relative z-10 flex flex-col items-center shrink-0">
        <p className="text-[10px] font-medium tracking-[0.1em] text-gray-600 mb-3 uppercase">Powered by deVee Boutique Label</p>
